...
describe('node/http/rateLimit', function () {
  describe('getXForwardedForIp', function () {
    ...
    it('should obtain client-ip ipv6', function () {
      const req = { headers: { 'x-forwarded-for': '2607:f0d0:1002:51::4, 127.0.0.1' } }
      const result = getXForwardedForIp(req)
      assert.strictEqual(result, '2607:f0d0:1002:51::4')
    })

    it('should reject if not ip', function () {
      const req = { headers: { 'x-forwarded-for': 'foobar, 1.2.3.4, 127.0.0.1' } }
      const result = getXForwardedForIp(req)
  ...
  describe('rateLimit', function () {
    ...
    it('should rate-limit', async function () {
      const max = 2
      const app = http.createServer(connect(rateLimit({ headers: true, max, timeoutSec: 60 }), echo, final))
      const headers = { 'X-Forwarded-For': '1.2.3.4, 127.0.0.1' }

      const rtest = ({ status = 200, remain, retryAfter }) => request(app)
        .get('/')
        .set(headers)
        .expect(status)
        .then(({ headers }) => {
      ...
      await rtest({ status: 429, remain: 0, retryAfter: 'Thu, 01 Jan 1970 00:02:00 GMT' })
    })
  })
})
