...
const finalError = (err, _req, res, _next) => {
  console.log(err)
  res.statusCode = 500
  res.end()
}
...
describe('node/http/route', function () {
  ...
  it('shall route GET', function () {
    return request(app).get('/').expect(200, 'GET 1 GET 2 ')
  })

  it('shall route POST', function () {
    return request(app).post('/').expect(200, 'POST 3 ')
  })

  it('shall route PUT', function () {
    return request(app).put('/').expect(200, 'PUT 4 ')
  })
